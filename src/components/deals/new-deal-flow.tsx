...
export function NewDealFlow() {
  ...
  return <div className={`new-deal-page${step === "summary" || step === "submitting" ? " summary-step" : ""}`}><section className="new-deal-heading"><p className="eyebrow">יצירת עסקה</p><h1>{step === "verify" ? "אימות נתוני הצ׳קים" : step === "summary" || step === "submitting" ? "סיכום העסקה" : "עסקה חדשה"}</h1><p>{step === "verify" ? "עברו על הנתונים שנקראו מהצ׳קים ואשרו שהם נכונים לפני שליחת העסקה." : step === "summary" || step === "submitting" ? "בדקו את פרטי העסקה לפני השליחה ל־ICAN." : "העלאת צ׳קים ליצירת עסקה חדשה"}</p></section><StepIndicator current={stepNumber} />
    ...
    {step === "summary" || step === "submitting" ? <div className="summary-review"><section className="summary-overview" aria-labelledby="summary-overview-title"><header className="summary-section-heading"><div><p>עיקרי העסקה</p><h2 id="summary-overview-title">תמונת מצב לפני שליחה</h2></div><span>כל הצ׳קים אומתו</span></header><div className="summary-metrics"><article><span>מספר צ׳קים</span><strong>{checks.length}</strong></article><article className="summary-total"><span>סה״כ עסקה</span><strong dir="ltr">{formatCurrency(totalAmount)}</strong></article><article><span>מועד קרוב</span><strong dir="ltr">{dueDates[0] ? formatDate(dueDates[0]) : "—"}</strong></article><article><span>מועד רחוק</span><strong dir="ltr">{dueDates.at(-1) ? formatDate(dueDates.at(-1)!) : "—"}</strong></article></div></section><section className="summary-checks" aria-labelledby="summary-checks-title"><header className="summary-section-heading"><div><p>בדיקה אחרונה</p><h2 id="summary-checks-title">פירוט הצ׳קים</h2></div><span>{checks.length} צ׳קים בעסקה</span></header><div className="summary-check-list">{checks.map((check, index) => <article className="summary-check-card" key={check.id}><header><div className="summary-check-number"><span>צ׳ק</span><strong>{index + 1}</strong></div><div><h3>פרטי צ׳ק {index + 1}</h3><p>הנתונים שנקראו ואושרו בשלב האימות</p></div><span className="summary-verified"><b aria-hidden="true">✓</b> אומת</span></header><dl><div><dt>מספר צ׳ק</dt><dd dir="ltr">{check.ocrData?.checkNumber || "—"}</dd></div><div><dt>סכום</dt><dd dir="ltr">{formatCurrency(Number(check.ocrData?.amount))}</dd></div><div><dt>תאריך פירעון</dt><dd dir="ltr">{formatDate(check.ocrData?.dueDate ?? "")}</dd></div><div><dt>שם מושך</dt><dd>{check.ocrData?.drawerName || "—"}</dd></div><div><dt>סטטוס אימות</dt><dd className="verified-value">אומת בהצלחה</dd></div></dl></article>)}</div></section><footer className="summary-actions"><div className="summary-action-copy"><p>העסקה מוכנה לשליחה</p><h2>אישור ושליחה ל־ICAN</h2><span>לאחר השליחה העסקה תועבר להמשך טיפול.</span></div><dl><div><dt>מספר צ׳קים</dt><dd>{checks.length}</dd></div><div><dt>סכום כולל</dt><dd dir="ltr">{formatCurrency(totalAmount)}</dd></div></dl><div className="summary-action-buttons"><button type="button" className="secondary-flow-button" onClick={() => setStep("verify")} disabled={step === "submitting"}>חזרה לאימות</button><button type="button" className="primary-flow-button" onClick={submitDeal} disabled={step === "submitting"}>{step === "submitting" ? "שולחים את העסקה..." : "שליחת העסקה ל-ICAN"}</button></div></footer></div> : null}
  </div>;
}
